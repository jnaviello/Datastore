...
class MainPage(webapp2.RequestHandler):
    def get(self):
        comment_name = self.request.get('wall_name', DEFAULT_WALL)
        if comment_name == DEFAULT_WALL.lower(): comment_name = DEFAULT_WALL
        ...
        # Render our page
        self.render("comments.html", sign_query_params, cgi.escape(comment_name),
                                         url, url_linktext)
...
class PostWall(webapp2.RequestHandler):
    def post(self):
        ...
        # ~1/second.
        wall_name = self.request.get('wall_name', DEFAULT_COMMENT_NAME)
        post = Post(parent=comment_key(wall_name))
